...
describe("computeProcessGeometry", () => {
	...
	it("computes edge pad, process-frame count, and warmup-frame count", () => {
		const geometry = computeProcessGeometry({
			totalFrames: 44100,
			fftSize: FFT,
			hopSize: HOP,
			sampleRate: 44100,
	...
	it("clamps the warmup window to totalFrames", () => {
		...
		const wholeFileWarmup = computeProcessGeometry({
			totalFrames: 44100,
			fftSize: FFT,
			hopSize: HOP,
			sampleRate: 44100,
	...
	it("defaults the sample rate to 48000 when undefined", () => {
		const withDefault = computeProcessGeometry({
		...
		const explicit48k = computeProcessGeometry({
			totalFrames: 10_000_000,
			fftSize: FFT,
			hopSize: HOP,
			sampleRate: 48000,
...
describe("computeChunkWindow", () => {
	...
	it("clamps window bounds and adds carry frames on each side", () => {
		const window = computeChunkWindow({
			outStart: 40,
			chunkFrames: 20,
