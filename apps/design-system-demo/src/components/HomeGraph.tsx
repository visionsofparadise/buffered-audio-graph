...
// Squash factor — the canvas is wider than tall, so we use this on all polar
// distances to keep clusters within the visible bounds.
const V_SQUASH = 0.6;
...
// Offshoot clusters orbiting the main one.
...
// Anchor points (the labelled recents) sit further out as isolated outliers.
const ANCHOR_DIST = 400;
const ANCHOR_DIST_JITTER = 50;
const ANCHOR_X_CLAMP = 130; // keep room for labels
const ANCHOR_Y_CLAMP = 60;

// foreignObject box for a recent-project label (icon + name + time).
...
function buildLayout(recentsCount: number, seed: number): Layout {
	...
	// 1. Main central cluster — uniform disk sampling around canvas center.
	...
	// 2. Offshoot clusters at evenly-spaced angles around the main, jittered
	// so they don't look mechanically placed.
	...
	// 3. Anchor outliers, one per recent project. Spread around the canvas at
	// angles offset from the offshoot clusters so they read as separate outliers.
	...
	for (let recentIndex = 0; recentIndex < recentsCount; recentIndex++) {
		...
		// Clamp into canvas with room for labels.
	...
	// Within the main cluster — dense connectivity.
	connectKNearestWithinRange({ startIndex: 0, endIndex: MAIN_COUNT }, 4);

	// Within each offshoot cluster — full or near-full mesh, since they're small.
	offshootRanges.forEach((range) => {
		connectKNearestWithinRange(range, 3);
	});

	// Bridge each offshoot cluster to the main cluster — take the N shortest
	// cross-cluster pairs so each offshoot is multiply-attached.
	...
	// Inter-offshoot bridges: connect each offshoot to its single nearest neighbor
	// offshoot via the closest cross-cluster edge. Adds richness without making
	// the layout chaotic.
	...
	// Each anchor outlier connects via a single long bridge to whichever
	// non-anchor point is closest.
	...
	// 5. Annotations: label sits radially outward from canvas center past the
	// anchor.
	...
	for (let recentIndex = 0; recentIndex < recentsCount; recentIndex++) {
		...
		// Compute outward direction from canvas center.
...
export function HomeGraph({ recents, onOpenRecent }: Props) {
	// Fresh random seed per mount — the home screen re-rolls its decoration
	// each time you land on it.
	...
	// Drive drift via direct DOM updates inside rAF to avoid React reconciliation
	// per frame. Anchors stay still so annotations don't visibly shift.
	useEffect(() => {
		let rafId = 0;
	...
	return (
		...
		>
			...
			{/* Tether lines from anchors to their labels. Anchors don't drift, so
			    these stay static. */}
			...
			{/* Anchor highlight ring on hover. */}
			{layout.annotations.map((ann) => {
				const point = layout.points[ann.pointIndex];
				const recent = recents[ann.recentIndex];
			...
			{/* Annotation labels. The whole group is the hover/click target; an
			    invisible hit-circle around the anchor lets hovering the point
			...
			{layout.annotations.map((ann) => {
				...
				return (
					<g key={`label-${recent.id}`}>
						...
						{/* Label — icon + name + time, laid out with HTML flex inside
						    a foreignObject so a wide icon never overlaps the text. */}
						<foreignObject
						...
						>
							<div
								className={cn(
									// Mirror the icon to the anchor side so the whole label
									// flips together — icon, name, and time all aligned.
